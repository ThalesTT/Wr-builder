...
export function MyBuilds() {
  ...
  const [itemCatalog, setItemCatalog] = useState<ItemData[]>([]);

  const { data: itensJson } = useFetchData<ItensJson>('/data/itens.json');

  useEffect(() => {
    if (itensJson) {
      setItemCatalog(itensJson.itens);
    }
  }, [itensJson]);

  // MyBuilds.tsx (Função de Ajuda)

  /**
   * Analisa a URL salva (ex: /itens/garen?ids=1,2,3&bd=Minha%20Build)
   * e retorna um array de IDs de itens.
   */
